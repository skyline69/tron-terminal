// Filled circle that expands from the cursor when it changes
//
// sonic_boom_cursor.glsl from ghostty-cursor-shaders by Sahaj Bhatt.
...
const MAX_RADIUS: f32 = 0.06; // Max radius in normalized coords (0.5 = 1/4 screen height)
...
fn shade(uv: vec2<f32>, frag_coord: vec2<f32>) -> vec4<f32> {
    ...
    if (is_mode_change > 0.0 || TRIGGER_ON_MOVE) && is_animating > 0.0 {
        ...
        let sdf_circle = dist - ripple_radius;

        // Antialias (1-pixel width in normalized coords)
        let anti_alias_size = norm(vec2<f32>(BLUR, BLUR), 0.0).x;
        let ripple = (1.0 - smoothstep(-anti_alias_size, anti_alias_size, sdf_circle)) * fade_amount;
