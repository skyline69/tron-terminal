// Circular ring ripple that expands from the cursor when it changes
//
// ripple_cursor.glsl from ghostty-cursor-shaders by Sahaj Bhatt.
...
const MAX_RADIUS: f32 = 0.05; // Max radius in normalized coords (0.5 = 1/4 screen height)
...
//   1.0 - exponentialDecayPulse return 1.0 - exp(-3.0 * t) * sin(t * 3.1415916);
fn fade(t: f32) -> f32 {
    return 1.0 - t * (2.0 - t);
}
...
fn shade(uv: vec2<f32>, frag_coord: vec2<f32>) -> vec4<f32> {
    ...
    if (is_mode_change > 0.0 || TRIGGER_ON_MOVE) && is_animating > 0.0 {
        ...
        // RIPPLE CALCULATION
        let ripple_radius = eased_progress * MAX_RADIUS;

        let fade_amount = fade(ripple_progress);

        // Calculate distance from frag to cursor center
        let dist = distance(vu, center_cc);

        let sdf_ring = abs(dist - ripple_radius) - RING_THICKNESS * 0.5;

        // Antialias (1-pixel width in normalized coords)
        let anti_alias_size = norm(vec2<f32>(BLUR, BLUR), 0.0).x;
        let ripple = (1.0 - smoothstep(-anti_alias_size, anti_alias_size, sdf_ring)) * fade_amount;
