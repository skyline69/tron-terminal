// Rectangular ring ripple that expands from the cursor when it changes
//
// ripple_rectangle_cursor.glsl from ghostty-cursor-shaders by Sahaj Bhatt.
...
const BLUR: f32 = 1.0; // Blur level in pixels
...
fn shade(uv: vec2<f32>, frag_coord: vec2<f32>) -> vec4<f32> {
    ...
    if (is_mode_change > 0.0 || TRIGGER_ON_MOVE) && is_animating > 0.0 {
        ...
        let sdf_rect_ring = abs(sdf_rectangle(vu, center_cc, half_size_cc)) - RING_THICKNESS * 0.5;
